...
export default class Scroller {
   ...
   */
  __isDecelerating: number | false = false;
  __interruptedAnimation: boolean = false;

  /**
   * {Boolean} Smoothly animating the currently configured change
   */
  __isAnimating: number | false = false;
  ...
  __refreshActive: boolean = false;

  /** {Function} Callback to execute on activation. This is for signalling the user about a refresh is about to happen when he release */
  __refreshActivate: (() => void) | null = null;

  /** {Function} Callback to execute on deactivation. This is for signalling the user about the refresh being cancelled */
  __refreshDeactivate: (() => void) | null = null;

  /** {Function} Callback to execute to start the actual refresh. Call {@link #refreshFinish} when done */
  __refreshStart: (() => void) | null = null;

  __zoomComplete: (() => void) | null = null;
   ...
   */
  setPosition(left: number | null, top: number | null) {
    var self = this;
  ...
  zoomTo(
    level: number,
    animate: boolean,
    originLeft: number | null | void,
    originTop: number | null | void,
    callback: (() => void) | null,
  ) {
    var self = this;
  ...
  scrollTo(
    ...
    if (!self.options.scrollingX) {
      ...
      if (self.options.paging && left != null) {
        left = Math.round(left / self.__clientWidth) * self.__clientWidth;
      } else if (self.options.snapping && left != null) {
    ...
    if (!self.options.scrollingY) {
      ...
      if (self.options.paging && top != null) {
        top = Math.round(top / self.__clientHeight) * self.__clientHeight;
      } else if (self.options.snapping && top != null) {
   ...
   */
  doTouchStart(touches, timeStamp: number) {
   ...
   */
  doTouchMove(touches: [], timeStamp: number, scale: number) {
    // Array-like check is enough here
    if (touches.length === null) {
      throw new Error('Invalid touch list: ' + JSON.stringify(touches));
    }
   ...
   */
  __computeScrollMax(zoomLevel: number | null | void) {
    var self = this;
   ...
   */
  __stepThroughDeceleration(render: () => void) {
    var self = this;
    ...
    if (!self.options.bouncing) {
      var scrollLeftFixed = Math.max(
        Math.min(self.__maxDecelerationScrollLeft || 0, scrollLeft),
        self.__minDecelerationScrollLeft || 0,
      );
      ...
      var scrollTopFixed = Math.max(
        Math.min(self.__maxDecelerationScrollTop || 0, scrollTop),
        self.__minDecelerationScrollTop || 0,
      );
    ...
    if (!self.options.paging) {
      ...
      self.__decelerationVelocityX =
        (self.__decelerationVelocityX || 0) * frictionFactor;
      self.__decelerationVelocityY =
        (self.__decelerationVelocityY || 0) * frictionFactor;
    }

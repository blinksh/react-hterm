...
lib.f.getStack = function(ignoreFrames = 0, count = undefined) {
  ...
  else count = lib.f.clamp(count, 0, max);
...
lib.PreferenceManager.prototype.importFromJson = function(
  ...
  const onWriteStorage = () => {
    if (--pendingWrites < 1) {
      if (opt_onComplete) opt_onComplete();

      // We've delayed updates to the child arrays, so flush them now.
      for (let name in json)
        if (name in this.childLists_) this.set(name, this.get(name));
...
lib.wc.charWidthDisregardAmbiguous = function(ucs) {
  ...
  if (ucs < 0x7f) {
    ...
    /* if (ucs < 0x20) */ else return lib.wc.controlWidth;
  }
...
lib.resource.add('libdot/changelog/version', 'text/plain', '2018-10-24');

lib.resource.add('libdot/changelog/date', 'text/plain', '1.24');
...
lib.resource.add(
  'hterm/concat/date',
  'text/plain',
  'Mon, 26 Nov 2018 08:50:09 +0000',
);

lib.resource.add('hterm/changelog/version', 'text/plain', '2018-10-24');

lib.resource.add('hterm/changelog/date', 'text/plain', '1.82');
...
hterm.Terminal.prototype.syncCursorPosition_ = function() {
  ...
  if (this.accessibilityReader_.accessibilityEnabled) {
    ...
    const cursorLineText = this.screen_.rowsArray[
...
hterm.Terminal.prototype.displayImage = function(options, onLoad, onError) {
  ...
  if (options.inline) {
    ...
    const img = /** @type {!HTMLImageElement} */ (this.document_.createElement(
      'img',
    ));
    img.src = options.uri;
...
hterm.TextAttributes.prototype.syncColors = function() {
  ...
  if (this.faint) {
    const colorToMakeFaint = getDefaultColor(
      this.foreground,
  ...
  if (this.underlineSource == this.SRC_DEFAULT) this.underlineColor = '';
...
hterm.VT.CSI['m'] = function(parseState) {
  ...
  for (var i = 0; i < parseState.args.length; i++) {
    ...
    if (arg < 30) {
      ...
      // Underline coloring.
      const result = this.parseSgrExtendedColors(parseState, i, attrs);
      if (result.color !== undefined) attrs.underlineSource = result.color;
      i += result.skipCount;
    } else if (arg == 59) {
